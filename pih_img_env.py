...
class PIHImgEnv(gym.Env):
    ...
    def _setup(self):
        ...
        self.block = self.add_box((256, 300), 120, 30)
        self.hole = self.add_hole((100, 100), 0)
    ...
    def box_coords(self, center, l, w):
    ...
    def add_hole(self, position, angle, scale=30, color='LightSlateGray', mask=pymunk.ShapeFilter.ALL_MASKS()):
        ...
        lcenter = [position[0] - (hole_w * scale), position[1]] # warn: doesnt account for wall thickness
        vertices1 = self.box_coords(lcenter, length * scale, wall_w * scale)
        inertia1 = pymunk.moment_for_poly(mass, vertices=vertices1)

        rcenter = [position[0] + (hole_w * scale), position[1]]
        vertices2 = self.box_coords(rcenter, length * scale, wall_w * scale)
        ...
        shapes = []
        cog = None
        for verts in [vertices1, vertices2, vertices3]:
        ...
        body.center_of_gravity = cog/len(shapes)
        body.position = position
        body.angle = angle
        body.friction = 1
